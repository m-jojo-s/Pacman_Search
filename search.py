...
def depthFirstSearch(problem):
	...
	while not stack.isEmpty():
		stateAt = stack.pop()
		if stateAt[0] in visited:
			continue
		direction = stateAt[1]
		visited.append(stateAt[0])
		
		if problem.isGoalState(stateAt[0]):
			directions = stateAt[1]
			break
		for successor in problem.getSuccessors(stateAt[0]):
			if successor[0] not in visited:
				stack.push( (successor[0], [successor[1]] + direction ) )
...
class _RecursiveDepthFirstSearch(object):
	...
	def RecursiveDepthFirstSearchHelper(self, node):
		...
		for successor in self.problem.getSuccessors(node[0])[::-1]:
			if successor[0] in self.explored: continue
			reachable = self.RecursiveDepthFirstSearchHelper(successor)
			if reachable:
				if len(node) > 2:
					self.actions.append(node[1])
				return True
...
def depthLimitedSearch(problem, limit = 210):
	...
	while not stack.isEmpty():
		...
		if len(stateAt[1]) >= limit:
			print("continuing for len:", len(stateAt[1]))
			continue
...
class _RecursiveDepthLimitedSearch(object):
	...
	def RecursiveDepthLimitedSearchHelper(self, node):
		...
		for successor in self.problem.getSuccessors(node[0])[::-1]:
			...
			reachable = self.RecursiveDepthLimitedSearchHelper(successor)
			if reachable:
				if len(node) > 2:
					self.actions.append(node[1])
				return True
...
def breadthFirstSearch(problem):
	...
	while not queue.isEmpty():
		...
		for successor in problem.getSuccessors(stateAt[0]):
			if successor[0] not in visited:
				queue.push( (successor[0], [successor[1]] + direction ) )
...
def uniformCostSearch(problem):
	...
	stateAt = (problem.getStartState(),[])
	priorityQueue.push(stateAt, 0)
	visited = []
	directions = []
 
	while not priorityQueue.isEmpty():
		stateAt, cost = priorityQueue.pop()
		if stateAt[0] in visited:
			index = priorityQueue.item_present_with_higher_pathCost(stateAt, cost)
			if index != None:
				priorityQueue.Update_priority(stateAt, cost, index, 1)
			continue
		direction = stateAt[1]
		visited.append(stateAt[0])
		...
		for successor in problem.getSuccessors(stateAt[0]):
			if successor[0] not in visited:
				priorityQueue.push( (successor[0], [successor[1]] + direction ) , successor[-1] + cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
	...
	"Start of Your Code"
	priorityQueue = util.PriorityQueue()
	stateAt = (problem.getStartState(),[], 0)
	priorityQueue.push(stateAt, heuristic(stateAt[0], problem) + stateAt[-1])
	visited = []
	directions = []
 
	while not priorityQueue.isEmpty():
		stateAt, fn = priorityQueue.pop()
		cost = stateAt[-1]
		if stateAt[0] in visited:
			index = priorityQueue.item_present_with_higher_pathCost(stateAt[0], fn)
			if index != None:
				priorityQueue.Update_priority(stateAt, fn, index, 1)
			continue
		direction = stateAt[1]
		visited.append(stateAt[0])
		...
		for successor in problem.getSuccessors(stateAt[0]):
			if successor[0] not in visited:
				priorityQueue.push( (successor[0], [successor[1]] + direction, successor[-1] + cost), successor[-1] + cost + heuristic(successor[0], problem))
